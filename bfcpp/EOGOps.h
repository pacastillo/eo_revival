...
public:
	...
	virtual void operator()( EOT& _eo ) const {
      ...
      std::uniform_int_distribution<int> rng_dist(0, _eo.length() -1 ) ;
	  unsigned pos1 = rng_dist(rng),
	    pos2 = rng_dist(rng);
...
#define min _MIN
  ...
  virtual void operator()( EOT& _eo, EOT& _eo2 ) const {
    ...
    std::random_device rd;
    std::default_random_engine rng(rd());
    std::uniform_int_distribution<int> rng_dist(0, max_size ) ;

    // select ranges of bits to swap
    do {
      unsigned bit = rng_dist(rng);
      if (points[bit]) {
	continue;
      }
      else {
...
#define min _MIN
  ...
  virtual void operator()( EOT& _eo1, EOT& _eo2 ) const {
    ...
    std::random_device rd;
    std::default_random_engine rng(rd());
    std::uniform_int_distribution<int> rng_dist(0, max_genes ) ;
